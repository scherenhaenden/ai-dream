...
@Component({
  ...
  template: `
    ...
    <section class="surface empty-surface"><div class="empty-illustration">{{ symbol }}</div><h2>{{ payload() !== null ? 'Local data received' : !api.connected() ? 'Waiting for the local backend' : dataPath ? 'Ready for local data' : 'Feature not connected yet' }}</h2><p>{{ payload() !== null ? 'This response came directly from your local AI Dream service.' : !api.connected() ? 'Connect the AI Dream local API to load real system data here. Nothing is simulated.' : connectedText }}</p><div class="endpoint"><span class="endpoint-dot"></span><code>{{ endpoint }}</code><span class="endpoint-status">{{ payload() !== null ? 'Data received' : !api.connected() ? 'Not reachable' : dataPath ? 'Not loaded' : 'Planned' }}</span></div>@if (payload() !== null) {<pre class="data-payload">{{ payload() | json }}</pre>} @if (loadError()) {<p class="error-line">{{ loadError() }}</p>} @if (!api.connected()) {<button class="secondary-button" (click)="api.check()">Retry connection <span>↻</span></button>} @else if (dataPath && payload() === null) {<button class="secondary-button" (click)="load()">Load from local API <span>↻</span></button>}</section>
