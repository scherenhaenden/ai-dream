...
- Optional offline voice output via `espeak-ng`/`espeak` with a responsive Stop speaking control; configurable-duration microphone recording and local audio-file transcription via `arecord`, `whisper-cli`/`whisper-cpp`, and a selectable local Whisper model.
